...
json_data = 'test_sideview_output.json'
#view = "Top"
...
for i in range(1):
    ...
    for obj in data:
        ...
        img_path.append((path,image_name,conf))
    ...
    #else:
    csv_filename = "test_sideview_pixels.csv"
    ...
    for i in range(0, l):
        if img_path[i][2]>0.7:
            img = cv.cvtColor(cv.imread(img_path[i][0]), cv.COLOR_BGR2RGB)
            #img = img_copy
            p, q, r = img.shape
            ...
            with open(csv_filename, 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow([img_path[i][1],xywh_list[i][4], total_pixel_count, sum_rowwise_pixel_count, max_rowwise_pixel_count, w0+h0, h0])

            #json_data = 'side_view_output.json'
            #view = "Side"
